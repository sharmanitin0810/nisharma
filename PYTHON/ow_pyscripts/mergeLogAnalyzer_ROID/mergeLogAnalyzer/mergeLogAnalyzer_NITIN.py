...
def extractTime(LineEntry, tzd):
    ...
    datePattern2 = re.compile(
        "\w{3} \w{3}\s{1,2}\d{1,2} \d{2}:\d{2}:\d{2} \w{3,4} \d{4}")
            ...
            if (currentBeam > 0):
                for nonVisibleBeam in reversed(range(currentBeam)):
                    ...
                    if (satBeam not in UTSatBeamRecords.keys()):
                        ...
                        print(
                            f"summaryEstimation:{satBeam}:{UTSatBeamRecords[satBeam]} ")
        if (newBeam < 15):
...
def estimateSummaryState(list_ind, entryString, UTSatBeamRecords, currentSatBeam, HOPredictionSatBeam):
    if (list_ind == 0):
        if ("_HO" in entryString):
            ...
            checkAndUpdateNotVisibleBeams(
                UTSatBeamRecords, HOPredictionSatBeam, newSat, newBeam)
            ...
            if ((HOPredictionSatBeam not in UTSatBeamRecords.keys()) or (UTSatBeamRecords[HOPredictionSatBeam] == "NOT_VISIBLE")):
                ...
                print(
                    f"summaryEstimation:{HOPredictionSatBeam}:{UTSatBeamRecords[HOPredictionSatBeam]}, entryString:{entryString} ")
    elif (list_ind == 1):
        if (currentSatBeam != 0):
            if ("from" in entryString):
                if (UTSatBeamRecords[currentSatBeam] == "PING_FAIL") or (UTSatBeamRecords[currentSatBeam] == "PING_PASS_FAIL"):
                    ...
                    print(
                        f"summaryEstimation:{currentSatBeam}:{UTSatBeamRecords[currentSatBeam]}, entryString:{entryString} ")
        ...
        if ("SystemInformationBlockType1" in entryString):
            ...
            newBeam = int(re.search("beam:(\d+),", entryString).group(1))
            checkAndUpdateNotVisibleBeams(
                UTSatBeamRecords, currentSatBeam, newSat, newBeam)
            ...
            beam = int(re.search("Physical Cell ID = (\d+)",
                                 entryString).group(1)) % 32
            if ("targetPhysCellId" in entryString):
                targetBeam = int(
                    re.search("targetPhysCellId (\d+)", entryString).group(1)) % 32
                if (",satellite-identity" in entryString):
                    targetSat = re.search(
                        ",satellite-identity \'(.*?)\'", entryString).group(1)
                    targetSat = targetSat.replace(" ", "")
                    targetSat = int(targetSat, 2)
                else:
                    targetSat = currentSatBeam//16
                checkAndUpdateNotVisibleBeams(
                    UTSatBeamRecords, currentSatBeam, targetSat, targetBeam)
            ...
            if (currentSatBeam != 0):
                beam = int(re.search("Cell ID = (\d+)",
                                     entryString).group(1)) % 32
                print(
                    f"beam:{beam} currentSatBeam:{currentSatBeam} HOPredictionSatBeam:{HOPredictionSatBeam}")
...
def createMergeFile(ping_file, sortedAlarms, alarmFilterList, modem_logs, prediction_file_path):
    ...
    if (prediction_file_path != ""):
        ...
        HOPredictionEntryTime = parser.parse(
            HOPredictionEntryTime + "UTC", tzinfos=tzd)
        HOPredictionEntryIndex = 0
        heapOfLeastElementOfAllFiles.append((HOPredictionEntryTime, 0, HOPredictionEntryIndex, "HO PREDICTION: "+HOPredictionEntry['SimUTCTime'] + ", " + HOPredictionEntry['HoType'] + ", Sat = " + HOPredictionEntry[
                                            'Sat'] + ", Beam = " + HOPredictionEntry['Beam'] + ", gnId = " + HOPredictionEntry['gnId'] + ", fwdChan = " + HOPredictionEntry['fwdChan'] + ", revChan = " + HOPredictionEntry['revChan'] + ", Info = " + HOPredictionEntry['Info']+"\n"))
    else:
        csvFile = None
    ...
    pingEntryIndex = 0
    heapOfLeastElementOfAllFiles.append(
        (pingTime, 1, pingEntryIndex, "PING          : "+PingLineEntry))

    if(len(sortedAlarms) > 0):
        iterSortedAlarms = iter(sortedAlarms)
        alarmLineEntry = next(iterSortedAlarms)
        entryToWrite = alarmLineEntry[1]["formattedRaisedTime"] + ", ClearTime:" + alarmLineEntry[1]["formattedClearTime"] + ", alarmID:" + str(alarmLineEntry[1]["alarmId"]) + ", alarmName:" + alarmLineEntry[1]["alarmName"] + ", reportingEntity:" + alarmLineEntry[1][
            "reportingEntityName"] + ", faultyEntity:" + alarmLineEntry[1]["faultyEntityName"] + ", entityId: " + alarmLineEntry[1]["entityId"] + ", supportingData:" + alarmLineEntry[1]["supportingData"] + ", Description:" + str(alarmLineEntry[1]["alarmDescription"])+"\n"
        alarmNum = alarmLineEntry[1]["alarmId"]
        alarmIndex = 0
        alarmTime = parser.parse(
            alarmLineEntry[1]["formattedRaisedTime"] + "UTC", tzinfos=tzd)
        ...
        try:
            while(alarmNum in alarmFilterList):
                ...
                alarmTime = parser.parse(
                    alarmLineEntry[1]["formattedRaisedTime"] + "UTC", tzinfos=tzd)
            heapOfLeastElementOfAllFiles.append(
                (alarmTime, 2, alarmIndex, "ALARM        : "+entryToWrite))
        except:
    ...
    if(modem_file != False):
        ...
        while(LogEntry == "" and exit_cond == False):
            ...
            if(LogEntry != ""):
                ...
                LogIndex = 0
                heapOfLeastElementOfAllFiles.append(
                    (LogTime, 3, LogIndex, "MDM          : "+LogEntry))
                ...
                if(modem_filenum == len(modem_logs)):
                    ...
                    modem_file = open(os.getcwd() + "\\" +
                                      modem_logs[modem_filenum])
                    print(modem_logs[modem_filenum])

    heapq.heapify(heapOfLeastElementOfAllFiles)

    while(heapOfLeastElementOfAllFiles):
        val, list_ind, element_ind, entryString = heapq.heappop(
            heapOfLeastElementOfAllFiles)
        writeFile.write(entryString)
        currentSatBeam, HOPredictionSatBeam = estimateSummaryState(
            list_ind, entryString, UTSatBeamRecords, currentSatBeam, HOPredictionSatBeam)
        if (list_ind == 0):
            try:
                HOPredictionEntry = next(reader)
                if (HOPredictionEntry != None):
                    ...
                    HOPredictionEntryTime = parser.parse(
                        HOPredictionEntryTime + "UTC", tzinfos=tzd)
                    heapq.heappush(heapOfLeastElementOfAllFiles, (HOPredictionEntryTime, 0, HOPredictionEntryIndex, "HO PREDICTION: " + HOPredictionEntry['SimUTCTime'] + ", " + HOPredictionEntry['HoType'] + ", Sat = " + HOPredictionEntry[
                                   'Sat'] + ", Beam = " + HOPredictionEntry['Beam'] + ", gnId = " + HOPredictionEntry['gnId'] + ", fwdChan = " + HOPredictionEntry['fwdChan'] + ", revChan = " + HOPredictionEntry['revChan'] + ", Info = " + HOPredictionEntry['Info'] + "\n"))
            except:
                csvFile.close()
        elif (list_ind == 1):
            ...
            while(validTimeEntry == False):
                ...
                if (PingLineEntry != ""):
                    ...
                    if (timeString != ""):
                        ...
                        heapq.heappush(heapOfLeastElementOfAllFiles, (pingTime,
                                                                      1, pingEntryIndex, "PING         : "+PingLineEntry))
                else:
                    pingFile.close()
                    validTimeEntry = True
        elif (list_ind == 2):
            try:
                ...
                if(alarmLineEntry != None):
                    alarmNum = alarmLineEntry[1]["alarmId"]
                    entryToWrite = alarmLineEntry[1]["formattedRaisedTime"] + ", ClearTime:" + alarmLineEntry[1]["formattedClearTime"] + ", alarmID:" + str(alarmLineEntry[1]["alarmId"]) + ", alarmName:" + alarmLineEntry[1]["alarmName"] + ", reportingEntity:" + alarmLineEntry[1][
                        "reportingEntityName"] + ", faultyEntity:" + alarmLineEntry[1]["faultyEntityName"] + ", entityId: " + alarmLineEntry[1]["entityId"] + ", supportingData:" + str(alarmLineEntry[1]["supportingData"] or 'None') + ", Description:" + str(alarmLineEntry[1]["alarmDescription"] or 'None')+"\n"
                    ...
                    heapq.heappush(heapOfLeastElementOfAllFiles, (alarmTime,
                                                                  2, alarmIndex, "ALARM        : "+entryToWrite))
            ...
            while(LogEntry == "" and exit_cond == False):
                ...
                if(LogEntry != ""):
                    ...
                    LogTime = LogTime - timedelta(seconds=18)
                    LogIndex = LogIndex + 1
                    heapq.heappush(heapOfLeastElementOfAllFiles,
                                   (LogTime, 3, LogIndex, "MDM          : "+LogEntry))
                else:
                    modem_file.close()
                    modem_filenum = modem_filenum + 1
                    if(modem_filenum == len(modem_logs)):
                        ...
                        modem_file = open(
                            os.getcwd() + "\\" + modem_logs[modem_filenum])
                        print(modem_logs[modem_filenum])
    writeFile.close()
    return UTSatBeamRecords
    ...
    while(lineToWrite == ""):
        ...
        if("OW RRC OTA Packet" in logLine):
            if ("RRCConnectionRequest" in logLine):
                ...
                lineToWrite = lineToWrite + ", sat:" + \
                    str(cellIdentity >> 8) + ", beam:" + \
                    str(cellIdentity & 0xff) + ", " + logLine.lstrip()
                ...
                while (i < 17):
                    logLine = UTlog.readline()
                    i = i + 1
                ellipsoidPointWithAltitude = re.search(
                    "\'(.*?)\'", logLine).group(1)
                positionString = decodeLatLongAlt(ellipsoidPointWithAltitude)
                lineToWrite = lineToWrite + \
                    f"{positionString}" + logLine.lstrip()
...
def getStartEndTimeFromUTLog(dirpath, ut_index):
    UTLogFiles = fnmatch.filter(os.listdir(
        dirpath), "UT"+ut_index+"_Log_*.txt")
    startTime = None
    endTime = None
    if(len(UTLogFiles) < 1):
...
def fetchAlarm(alarmHistoryFile, alarmStartTimeStr, alarmEndTimeStr):
    global inputParams
    myclient = getSSHClient(str(inputParams[1]), str(
        inputParams[2]), str(inputParams[3]))

    command = "curl -i --insecure 'https://'" + \
        str(inputParams[4]) + \
        "'/gnems/login' -XPOST -H 'Accept: application/json, text/plain, */*' -H 'Content-Type: application/json;charset=utf-8' --data-binary '{\"username\":\"hughesemsadmin\",\"password\":\"Drinksunnyalike5}\",\"selDbName\":\"oneweb_config_01\"}'"
    ...
    command = "curl -i --insecure 'https://'"+str(inputParams[4])+"'/gnems/api/alarmhistory/filter' -XPOST -H 'Content-Type: application/json' -H 'Authorization: Bearer " + token + \
        "' -H 'selDbName: oneweb_config_01' --data-binary '{\"startDate\":\"" + alarmStartTimeStr + "\",\"endDate\":\"" + alarmEndTimeStr + \
        "\",\"severityList\":[],\"alarmIdList\":[],\"reportingEntityList\":[],\"faultyEntityList\":[],\"entityIdList\":[],\"clearedByList\":[],\"page\":0,\"size\":100}'"
    mystdin, mystdout, mystderr = myclient.exec_command(command, get_pty=True)
    ...
    if (size > 100):
        command = "curl -i --insecure 'https://'"+str(inputParams[4])+"'/gnems/api/alarmhistory/filter' -XPOST -H 'Content-Type: application/json' -H 'Authorization: Bearer " + token + \
            "' -H 'selDbName: oneweb_config_01' --data-binary '{\"startDate\":\"" + alarmStartTimeStr + "\",\"endDate\":\"" + alarmEndTimeStr + \
            "\",\"severityList\":[],\"alarmIdList\":[],\"reportingEntityList\":[],\"faultyEntityList\":[],\"entityIdList\":[],\"clearedByList\":[],\"page\":0,\"size\":" + \
            str(size)+"}'"
        mystdin, mystdout, mystderr = myclient.exec_command(
    ...
    for dirpath, dirs, files in os.walk('.'):
        ...
        for file in fnmatch.filter(files, str(ut_list[ut_index][1])+"_"+ut_list[ut_index][0]+"*.txt"):
            ...
            if (dateMatch != None):
                ...
                timeString = dateMatch.group(0)
                modemFileTime = datetime.strptime(
                    str(startTime.year)+"-"+timeString, '%Y-%m-%d.%H-%M-%S')
                print(
                    f"Timeline modemFileTime:{modemFileTime}, startTime:{startTime} endTime:{endTime}")
...
def summaryStatusConditionalFormat(workbook):
    ...
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'NOT_VISIBLE', 'format': format_light_gray})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'DETACHED', 'format': format_red})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'RLF', 'format': format_red})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'SIB_FAIL', 'format': format_red})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'PING_FAIL', 'format': format_biege})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'PING_PASS_FAIL', 'format': format_light_green})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'PING_PASS', 'format': format_dark_green})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'HO_SUCCESS', 'format': format_light_green})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'ATTACHED', 'format': format_light_green})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'HO_TRIGGERED', 'format': format_pink})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'ACQUIRED', 'format': format_light_blue})
    worksheet.conditional_format(1, 1, row, maxcol, {
                                 'type': 'text', 'criteria': 'containing', 'value': 'NOT_ACQ', 'format': format_turquoise})
        ...
        try:
            while (alarmLineEntry != None):
                if (alarmLineEntry[1]["alarmId"] == 32503):
                    ...
                    sap = int(re.search("SAP ID: (\d+),",
                                        alarmLineEntry[1]["supportingData"]).group(1))
...
def parseBasicConfig():
    ...
    inputParams.append(basic_config.get('BASIC_CONFIG', 'GNOC_START_TIME'))
...
def convertIsfToAscii(input_date_folder):

    global inputParams
    check_output("net use u: /delete")
    check_output("net use u: "+'\\\\' +
                 str(inputParams[5])+'\\'+str(inputParams[6])+'\\'+input_date_folder)
    os.chdir("t:")
    for dirpath, dirs, files in os.walk('.'):
        for file in fnmatch.filter(files, '*.isf'):
            filename = os.path.join(dirpath, file)
            print(filename)
            check_output("\"C:\\Program Files (x86)\\QUALCOMM\\QCAT 6.x\\Bin\\QCAT.exe\" -txt -filter=C:\\Users\\oneweb\\Documents\\QCAT\\QCAT_FILTERS\\OVERVIEW_with_ML1.txt "+filename, shell=True)
           #check_output("\"C:\\Program Files (x86)\\QUALCOMM\\QCAT 6.x\\Bin\\QCAT.exe\" -analyzer=\"User;Custom;RSRP and RSSI \""  + filename, shell=True)

    print("ISF to Txt. file conversion Completed ..!!")
    time.sleep(2)
...
def csv_from_excel():
    ...
    read_file.to_csv(r'UtBtHandoverSimulation.csv',
                     index=None, header=True, encoding='utf-8')
    print("csv file genrated successfully ..!!")


def align_prediction_file():

    global inputParams
    #check_output("net use u: /delete")
    #check_output("net use u: "+'\\\\' +str(inputParams[5])+'\\'+str(inputParams[6])+'\\'+input_date_folder)

    print("Going to align the predition file with GNOC start time.. ")
    wb = load_workbook(filename='UtBtHandoverSimulation.xlsx')
    ws = wb.worksheets[1]
    ws['B1'] = str(inputParams[7])
    time.sleep(4)
...
if __name__ == "__main__":
    ...
    convert_choice = input(
        "Do you want to convert ISF files to Text Files ? (Yes/No)")
    if convert_choice == 'Yes':
        print("Converting ISF files to Text Files..")
        convertIsfToAscii(sys.argv[1])
    else:
        print("Skipping ISF file conversions...Proceding Further..")

    if (len(sys.argv)-1) < 1:
        print(
            "The script is called with zero arguments\n format: {} <YYYY-MM-DD>".format(sys.argv[0]))
        sys.exit(1)
    else:
        date = sys.argv[1]
    ...
    try:
        ...
        pass
    check_output("net use u: "+'\\\\' +
                 str(inputParams[5])+'\\'+str(inputParams[6])+'\\'+date)
    os.chdir("t:")
    ...
    for dirpath, dirs, files in os.walk('.'):
        for file in fnmatch.filter(files, 'UtBtHandoverSimulation_ho.csv'):
            if ("croatia" in dirpath):
                ...
                prediction_file["LAB_ROID"] = os.path.join(dirpath, file)
    ...
    row = 0
    firstPingFile = True
    ut_list = {"1": (str(inputParams[7]), str(inputParams[0])), "2": (
        "20", "PMO"), "3": ("30", "PMO"), "4": ("40", "PMO")}
    satSapMap = {}
        ...
        for dirpath, dirs, files in os.walk('.'):
            for file in fnmatch.filter(files, 'UT'+ut_index+'_PING_*'):
                ...
                if (dirpath != prev_dirpath):
                    startTime, endTime = getStartEndTimeFromUTLog(
                        dirpath, ut_index)
                    if (startTime == None):
                        continue
                    ...
                    alarmStartTimeStr = alarmStartTime.strftime(
                        '%Y-%m-%d %H:%M:%S')
                    alarmStartTimeInMinutesStr = alarmStartTime.strftime(
                        '%Y-%m-%d %H:%M')
                    alarmHistoryFile = os.path.join(
                        dirpath, "alarmHistoryData"+str(alarmStartTimeInMinutesStr)+".json")
                    alarmHistoryFile = alarmHistoryFile.replace(":", "-")
                    if (os.path.exists(alarmHistoryFile) == False):
                        alarmEndTimeStr = alarmEndTime.strftime(
                            '%Y-%m-%d %H:%M:%S')
                        fetchAlarm(alarmHistoryFile,
                                   alarmStartTimeStr, alarmEndTimeStr)
                    with open(alarmHistoryFile, "r") as alarmFile:
                        data = json.load(alarmFile)
                        sortedAlarms = sorted(data['data']['alarmHistoryData'].items(
                        ), key=lambda x: x[1]["raisedTime"], reverse=False)
                        if (firstPingFile == True):
                            satSapMap = fetchSatSapMap(sortedAlarms)
                            print(satSapMap)
                    modem_logs = filterModemLogs(
                        ut_list, ut_index, alarmStartTime, alarmEndTime)
                UTSatBeamRecords = createMergeFile(
                    ping_file, sortedAlarms, alarmFilterList, modem_logs, prediction_file[ut_list[ut_index][1]])
                if (firstPingFile == True):
